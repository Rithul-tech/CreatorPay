  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
      ...
      <div className={`rounded-xl p-6 flex flex-col justify-between space-y-6 border transition-colors duration-100 ${
        ...
        <div className="space-y-2" ref={countryRef}>
          ...
          <div className="relative">
            ...
            {isCountryOpen && (
              <div className={`absolute z-20 mt-1.5 w-full border rounded-lg shadow-xl overflow-hidden max-h-60 overflow-y-auto ${
                theme === 'dark' ? 'bg-[#1A1A1E] border-zinc-800' : 'bg-white border-zinc-200'
              }`}>
                <div className={`p-1 divide-y ${theme === 'dark' ? 'divide-zinc-800/40' : 'divide-zinc-200/40'}`}>
      ...
      <div className={`rounded-xl p-6 flex flex-col justify-between space-y-6 border transition-colors duration-100 ${
        ...
        <div className={`space-y-4 pt-4 border-t transition-colors duration-100 ${
          ...
          <div className="flex items-center justify-between text-xs py-1">
            ...
            <span className={`font-mono font-bold transition-colors duration-100 ${
              ...
              {activeCountry.symbol}{localRpm.toFixed(2)} / 1K views
            </span>
          </div>

          {/* Annual Projections row */}
          <div className="flex items-center justify-between text-xs py-1">
            <span className="text-zinc-500 font-medium">Projected Annual Value</span>
            <span className={`font-mono font-bold transition-colors duration-100 ${
              isZeroRevenueScenario 
                ? 'text-zinc-500' 
                : theme === 'dark' ? 'text-emerald-400' : 'text-emerald-600'
        ...
        {/* Contextual Ad Placement Block */}
        <div className="pt-2">
          <div className={`h-[90px] w-full border border-dashed rounded-lg flex flex-col items-center justify-center text-[9px] font-mono uppercase tracking-widest text-center transition-colors duration-100 ${
            theme === 'dark' 
              ? 'border-zinc-800 bg-zinc-900/10 text-zinc-600' 
              : 'border-zinc-200 bg-zinc-50 text-zinc-500'
          }`}>
            <span>[Ad Placement Area]</span>
            <span className="text-[8px] text-zinc-600 font-sans mt-0.5">300x250 Responsive Native Grid Slot</span>
          </div>
        </div>
